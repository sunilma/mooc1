...
const Person = ({person: {_id, name, number}, handleDelete}) => (
    <p key={_id}>{name}{" "}{number}<button onClick={() => handleDelete(_id)}>Delete</button></p>
);
...
const Persons = ({persons, filter, handleDelete}) => {
    if(persons.length > 0) {
        ...
        .map(person => <Person key={person._id} person={person} handleDelete={handleDelete} />)
    } else {
        return <p>Loading...</p>;
    }
}
  ...
  const addPerson = (event) => {
      ...
      if(isExists.length > 0) {
        if(window.confirm(`${newName} already exists in the phonebook, replace the old number with a new one?`)){
            updateOne(isExists[0]._id, newNumber)
            .then(response => {
                const temp = persons.filter(person => person._id !== response.data._id);
                response.data.number = newNumber;
                setPersons([...temp, response.data]);
            })
            .catch(error => {
              setMessage({msg:`${newName} has already been removed from the database`, type: 'error'});
              setTimeout(() => {
                  setMessage(null);
              }, 5000);
            });
        ...
        addOne(person).then(response => {
            ...
            console.log(error);
            setMessage({ msg: error.response.data, type: 'error'});
            setTimeout(() => {
              setMessage(null);
            }, 5000);
          });
  ...
  const handleDelete = (id) => {
    if (window.confirm("are you sure?")) { 
        removeOne(id).then(response => {
            const newPersons = persons.filter(person => person._id !== id);
